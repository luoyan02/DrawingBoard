...
public:
	...
	int GetWidth() { return this->frameGeometry().width(); }
	int GetHight() { return this->frameGeometry().height(); }
	void SetColor();
	void GetButtonColor();
	static QColor WColor;
	static int WWidth;
private:
	...
	//表示当前状态：画直线、多边形、椭圆
	enum PAINTER_STATE {
		NOT_DRAWING, DRAW_LINE, DRAW_POLYGON, DRAW_ELLIPSE, DRAW_ROTATE,  DRAW_SCALE, DRAW_CLIP, DRAW_CURVE, DRAW_RANDOM, CLEAR
	};
	...
	QPushButton* colorButton = new QPushButton("rgb");
	//随意画
...
private slots:
	void setWidth()
	{
		CG_Painter::WWidth = widthEdit->value();
	}
	...
	void state_to_draw() { setState(DRAW_RANDOM); }
	...
	void state_to_clear() { setState(CLEAR); }
};
